...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Edit Blog Sub-Category</h6>
    </div>
    <div class="card-body">
        <form method="POST" action="{{route('blog-sub-categories.update', $subCategory->id)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="blog_category_id" id="blog_category_id" class="form-control @error('blog_category_id') is-invalid @enderror" required>
                            ...
                            @foreach($categories as $id => $name)
                                <option value="{{$id}}" {{old('blog_category_id', $subCategory->blog_category_id) == $id ? 'selected' : ''}}>{{$name}}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        @if($subCategory->image)
                            <small class="form-text text-info">Current image: <img src="{{asset('uploads/blog/'.$subCategory->image)}}" class="img-fluid" style="max-width:100px" alt="{{$subCategory->sub_category_name}}"></small>
                        @endif
                        @error('image')
                            <span class="text-danger">{{$message}}</span>
                        @enderror
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <a href="{{route('blog-sub-categories.index')}}" class="btn btn-secondary">

...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Add Blog Sub-Category</h6>
    </div>
    <div class="card-body">
        <form method="POST" action="{{route('blog-sub-categories.store')}}" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="blog_category_id" id="blog_category_id" class="form-control @error('blog_category_id') is-invalid @enderror" required>
                            ...
                            @foreach($categories as $id => $name)
                                <option value="{{$id}}" {{old('blog_category_id') == $id ? 'selected' : ''}}>{{$name}}</option>
                            @endforeach
                        </select>

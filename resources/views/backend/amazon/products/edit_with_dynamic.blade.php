...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Edit Amazon Product</h6>
    </div>
    <div class="card-body">
        <form method="POST" action="{{route('amazon-products.update', $product->id)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="amazon_category_id" id="amazon_category_id" class="form-control @error('amazon_category_id') is-invalid @enderror" required>
                            ...
                            @foreach($categories as $category)
                                <option value="{{$category->id}}" {{old('amazon_category_id', $product->amazon_category_id) == $category->id ? 'selected' : ''}}>{{$category->category_name}}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="amazon_sub_category_id" id="amazon_sub_category_id" class="form-control @error('amazon_sub_category_id') is-invalid @enderror">
                            <option value="">Select Sub-Category</option>
                            @if($subCategories->isNotEmpty())
                                @foreach($subCategories as $subCategory)
                                    <option value="{{$subCategory->id}}" {{old('amazon_sub_category_id', $product->amazon_sub_category_id) == $subCategory->id ? 'selected' : ''}}>{{$subCategory->sub_category_name}}</option>
                                @endforeach
                            @endif
                        </select>
                        @error('amazon_sub_category_id')
                            <span class="text-danger">{{$message}}</span>
                        @enderror
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        @if($product->image)
                            <small class="form-text text-info">Current image: <img src="{{asset('uploads/amazon/'.$product->image)}}" class="img-fluid" style="max-width:100px" alt="{{$product->product_name}}"></small>
                        @endif
                        @error('image')
                            <span class="text-danger">{{$message}}</span>
                        @enderror

...
<script>
    $(document).ready(function() {
        ...
        // Initialize role-based permission state on page load
        var currentRole = $('select[name="role_id"]').val();
        if (currentRole == '1') { // Super Admin
            ...
            // Initialize select all state based on current permissions for non-super admin
            var totalEnabled = $('.permission-checkbox:not(:disabled)').length;
            var totalChecked = $('.permission-checkbox:not(:disabled):checked').length;
            $('#selectAll').prop('checked', totalEnabled === totalChecked && totalEnabled > 0);
